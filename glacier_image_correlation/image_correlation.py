...
def run_autoRIFT(img1, img2, skip_x=3, skip_y=3, min_x_chip=16, max_x_chip=64,
    ...
    m, n = obj.I1.shape
    xGrid = np.arange(obj.SkipSampleX + 10, min(n - obj.SkipSampleX, n), obj.SkipSampleX)
    yGrid = np.arange(obj.SkipSampleY + 10, min(m - obj.SkipSampleY, m), obj.SkipSampleY)
    
    xGrid = xGrid[xGrid < n]
    yGrid = yGrid[yGrid < m]

    obj.xGrid = np.int32(np.dot(np.ones((len(yGrid), 1)), np.reshape(xGrid, (1, len(xGrid)))))
    obj.yGrid = np.int32(np.dot(np.reshape(yGrid, (len(yGrid), 1)), np.ones((1, len(xGrid)))))

    noDataMask = np.invert(
        np.logical_and(
            obj.I1[:, np.clip(xGrid - 1, 0, n - 1)][np.clip(yGrid - 1, 0, m - 1), ] > 0,
            obj.I2[:, np.clip(xGrid - 1, 0, n - 1)][np.clip(yGrid - 1, 0, m - 1), ] > 0
        )
    )
...
def main():
    ...
    search_limit_x = search_limit_y = round(
        ((((img2_ds.time.isel(time=0) - img1_ds.time.isel(time=0)).dt.days) * 100) / 365.25).item()
    )
